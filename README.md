...
Electron 환경에서 BrowserRouter를 사용하면, URL 경로를 기준으로 라우팅이 이루어지게 됩니다. 하지만 이 경우, 라우팅 페이지를 가져오지 못하는 문제가 발생 했습니다. 이러한 문제는 Electron이 웹 페이지를 로드하는 방식과 관련이 있습니다.<br />
이를 해결하기 위해, HashRouter를 사용하는 방법을 선택했습니다. HashRouter는 URL의 해시[#] 부분을 기준으로 라우팅을 처리하기 때문에, 경로 변경이 페이지 전체의 리로딩을 유발하지 않으며, 흰 화면이 보이는 문제를 피할 수 있었습니다.
